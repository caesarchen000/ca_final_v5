...
def suite_prefetch_ghb(args: argparse.Namespace) -> int:
    ...
    # Golden checksums for correctness validation
    GOLDEN_CHECKSUMS = {
        "stream": "Checksum: 90112.000000",
        "vvadd": "Checksum: 67108864",
        "qsort": "Sorted: Yes, Checksum: 10235808",
        "towers": "Total moves: 262143",
        "mm": "Checksum: 89456640",
        "binary_search": "Found count: 5000",
        "pointer_chase": "Checksum: 52377600",
    }
    ...
    if result == 0:
        print("\n" + "="*80)
        print("Validating Correctness (Golden Output Comparison)")
        print("="*80)
        ...
        for bench_name, _ in benchmarks:
            golden_file = golden_dir / f"{bench_name}.golden"

            if not golden_file.exists():
                print(f"  ⚠ {bench_name:<14}: Golden file not found, skipping")
                continue
            ...
            for prefetcher in prefetchers:
                label = f"pref_{slugify(prefetcher['label'])}"
                bench_dir = output_dir / bench_name / label / "caches"
                simout_file = bench_dir / "simout.txt"

                if simout_file.exists():
                    with open(simout_file, 'r') as f:
                        simout_content = f.read()

                    # Extract golden section from simout
                    import re
                    match = re.search(r'=== GOLDEN_OUTPUT_START ===\n(.*?)\n=== GOLDEN_OUTPUT_END ===',
                                     simout_content, re.DOTALL)

                    if match:
                        # Compare only the data content (group 1), not the markers
                        actual_data = match.group(1)
                        golden_data = re.search(r'=== GOLDEN_OUTPUT_START ===\n(.*?)\n=== GOLDEN_OUTPUT_END ===',
                                               golden_output, re.DOTALL)
                        if golden_data and actual_data == golden_data.group(1):
                            status = "✓"
                        else:
                            status = "✗"
                            golden_pass = False
                    ...
                    pref_label = prefetcher['label'][:12]  # Truncate for display
                    print(f"  {status} {bench_name:<14} ({pref_label:<12})")
                else:
                    print(f"  ✗ {bench_name:<14} ({prefetcher['label']:<12}): simout.txt not found")
                    golden_pass = False

        print("="*80)
        if golden_pass:
            print("✓ All golden outputs MATCH - Implementation is CORRECT")
        else:
            print("✗ Golden output mismatch detected - Implementation may have BUGS")
            result = 1
        print()
...
def cmd_build(args: argparse.Namespace) -> int:
    ...
    # Ensure zlib.h is available in conda if using conda Python
    conda_prefix = os.environ.get("CONDA_PREFIX", "")
    if not conda_prefix:
        # Try to detect conda base
        conda_base = os.environ.get("CONDA_EXE", "")
        if conda_base:
            conda_prefix = os.path.dirname(os.path.dirname(conda_base))
    
    if conda_prefix:
        zlib_header = os.path.join(conda_prefix, "include", "zlib.h")
        if not os.path.exists(zlib_header):
            # Download zlib.h if missing (user doesn't have sudo for zlib1g-dev)
            try:
                ...
                print(f"[build] Downloaded zlib.h to {zlib_header}")
            except Exception as e:
                print(f"[build] Warning: Could not download zlib.h: {e}", file=sys.stderr)
    ...
    for build_target in targets_to_build:
        # Use full path to scons from conda
        scons_path = os.environ.get("SCONS_PATH", "scons")
        # Try to find scons in common locations
        if scons_path == "scons" and not os.path.exists("/usr/bin/scons"):
            conda_prefix = os.environ.get("CONDA_PREFIX", "")
            if conda_prefix:
                scons_candidate = os.path.join(conda_prefix, "bin", "scons")
                if os.path.exists(scons_candidate):
                    scons_path = scons_candidate
        
        # Set up environment for build
        build_env = os.environ.copy()
        # Use system compilers if available
        if os.path.exists("/usr/bin/gcc"):
            build_env["CC"] = "/usr/bin/gcc"
        if os.path.exists("/usr/bin/g++"):
            build_env["CXX"] = "/usr/bin/g++"
        # Ensure Python library path is set for conda Python
        conda_prefix = os.environ.get("CONDA_PREFIX", "")
        if conda_prefix:
            lib_path = os.path.join(conda_prefix, "lib")
            include_path = os.path.join(conda_prefix, "include")
            if "LD_LIBRARY_PATH" in build_env:
                build_env["LD_LIBRARY_PATH"] = f"{lib_path}:{build_env['LD_LIBRARY_PATH']}"
            else:
                build_env["LD_LIBRARY_PATH"] = lib_path
            # Add conda include path for headers (like zlib.h)
            if "CPATH" in build_env:
                build_env["CPATH"] = f"{include_path}:{build_env['CPATH']}"
            else:
                build_env["CPATH"] = include_path
            if "C_INCLUDE_PATH" in build_env:
                build_env["C_INCLUDE_PATH"] = f"{include_path}:{build_env['C_INCLUDE_PATH']}"
            else:
                build_env["C_INCLUDE_PATH"] = include_path
            if "CPLUS_INCLUDE_PATH" in build_env:
                build_env["CPLUS_INCLUDE_PATH"] = f"{include_path}:{build_env['CPLUS_INCLUDE_PATH']}"
            else:
                build_env["CPLUS_INCLUDE_PATH"] = include_path
            # Set PYTHON_CONFIG to use conda's python3-config (absolute path required)
            python3_config = os.path.join(conda_prefix, "bin", "python3-config")
            if os.path.exists(python3_config):
                build_env["PYTHON_CONFIG"] = os.path.abspath(python3_config)
        
        build_cmd = [scons_path, f"-j{jobs}", build_target]
        print(f"[build] Building {build_target} via: {' '.join(build_cmd)}")
        build_proc = subprocess.run(build_cmd, cwd=gem5_root, env=build_env)
