...
def run_gem5(
    ...
    # Set up environment for gem5 execution
    # Need to set LD_LIBRARY_PATH so gem5 can find Python shared libraries
    env = os.environ.copy()
    conda_prefix = os.environ.get("CONDA_PREFIX", "")
    if not conda_prefix:
        # Try to detect conda base from common locations
        conda_exe = os.environ.get("CONDA_EXE", "")
        if conda_exe:
            conda_prefix = str(Path(conda_exe).parent.parent)
        else:
            # Try default miniforge/anaconda location
    ...
    simout_file = case_dir / "simout.txt"
    with simout_file.open("w") as f:
        f.write(result.stdout)

    simerr_file = case_dir / "simerr.txt"
    with simerr_file.open("w") as f:
