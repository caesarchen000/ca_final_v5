    ...
    const CacheAccessor &cache)
{
    ...
    bool hasMatch = historyHelper.findPatternMatch(chronological, predicted, degree * 3);
    ...
    if (hasMatch) {
        ...
        if (chronological.size() >= 2 && chronological.back() != 0) {
            ...
            if (chronological.size() >= 3) {
                ...
                if (last == prev && last != 0) {
                    target_degree = degree + 3;  // More aggressive for consistent fallback
                } else {
    ...
    if (!predicted.empty() && prefetches_generated < target_degree) {
        ...
        if (primary_delta != 0) {
            ...
            unsigned max_chain = hasMatch ? target_degree * 4 : degree * 2;  // Chain more for high-confidence
    ...
    if (predicted.size() > 1 && prefetches_generated < target_degree && hasMatch) {
        ...
        if (secondary_delta != 0 && secondary_delta != predicted[0]) {
            ...
            unsigned max_secondary_chain = degree * 2;
